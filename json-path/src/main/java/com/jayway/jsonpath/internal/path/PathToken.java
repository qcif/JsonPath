	...
	void handleObjectProperty(String currentPath, Object model, EvaluationContextImpl ctx, List<String> properties) {
		...
		if (properties.size() == 1) {
			...
			if (propertyVal == JsonProvider.UNDEFINED) {
				// Conditions below heavily depend on current token type (and
				// its logic) and are not "universal",
				// so this code is quite dangerous (I'd rather rewrite it & move
				// to PropertyPathToken and implemented
				// WildcardPathToken as a dynamic multi prop case of
				// PropertyPathToken).
				// Better safe than sorry.
				assert this instanceof PropertyPathToken : "only PropertyPathToken is supported";

				if (isLeaf()) {
					if (ctx.options().equals(Option.WRITE_IF_KEY_NOT_EXIST)) {
					...
					if (!(isUpstreamDefinite() && isTokenDefinite())
						...
						// If there is some indefiniteness in the path and
						// properties are not required - we'll ignore
						// absent property. And also in case of exception
						// suppression - so that other path evaluation
